...
const Home = () => {
  ...
  const services = [
    ...
    },
    {
      ...
      link: '/services/OnSide',
      bgImage: serviceBackgrounds[2]
    },
    {
  ...
  return (
    <div className="w-full">
      ...
      <section className="relative h-[50vh] sm:h-[60vh] md:h-[70vh] lg:h-[90vh] overflow-hidden">
      ...
      {/* Services Section - Updated with Hover Effects */}
      <section className="py-10 sm:py-14 md:py-20 bg-gradient-to-b from-white to-gray-50">
        <div className="container mx-auto px-4 sm:px-6">
          <div className="text-center max-w-3lg mx-auto mb-10 sm:mb-12 md:mb-16">
            <div className="inline-flex items-center gap-1 sm:gap-2 mb-3 sm:mb-4 px-3 py-1 sm:px-4 sm:py-2 bg-blue-100 rounded-full">
              <div className="w-1 h-1 sm:w-2 sm:h-2 bg-blue-500 rounded-full"></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 md:gap-8">
            {services.map((service, index) => (
              <motion.div
                ...
                className="group relative rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 shadow-lg hover:shadow-xl md:hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 md:hover:-translate-y-2 overflow-hidden cursor-pointer"
                onMouseEnter={() => setActiveService(index)}
                whileHover={{ scale: 1.02 }}
              >
                {/* Background Image with hover effect */}
                <div 
                  className="absolute inset-0 bg-cover bg-center z-0 transition-transform duration-700 group-hover:scale-110 group-hover:rotate-1"
                  style={{ backgroundImage: `url(${service.bgImage})` }}
                >
                  <div className="absolute inset-0 bg-gradient-to-br from-black/80 via-black/60 to-black/40 group-hover:from-black/90 group-hover:via-black/70 group-hover:to-black/50 transition-all duration-500"></div>
                </div>
                
                {/* Gradient Overlay on Hover */}
                <div className={`absolute inset-0 bg-gradient-to-br ${service.color} opacity-0 group-hover:opacity-20 transition-opacity duration-500 z-0`}></div>
                
                <div className="relative z-10">
                  <div className={`p-3 sm:p-4 rounded-lg sm:rounded-xl bg-gradient-to-br ${service.color} inline-flex mb-4 sm:mb-6 transform group-hover:scale-110 group-hover:rotate-3 transition-transform duration-500`}>
                    <div className="text-white">{service.icon}</div>
                  </div>
                  
                  <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-white mb-3 sm:mb-4 group-hover:text-blue-200 transition-colors duration-300">
                    {service.title}
                  </h3>
                  <p className="text-white/90 mb-4 sm:mb-6 text-sm sm:text-base group-hover:text-white transition-colors duration-300">
                    {service.description}
                  </p>
                  ...
                  <ul className="space-y-2 sm:space-y-3 mb-6 sm:mb-8">
                    {service.features.map((feature, idx) => (
                      <motion.li 
                        key={idx} 
                        className="flex items-center gap-2 sm:gap-3 text-white/90 text-sm sm:text-base group-hover:text-white transition-colors duration-300"
                        whileHover={{ x: 5 }}
                        transition={{ type: "spring", stiffness: 300 }}
                      >
                        <div className="w-1 h-1 sm:w-2 sm:h-2 bg-white rounded-full flex-shrink-0 group-hover:bg-blue-300 group-hover:scale-125 transition-all duration-300"></div>
                        <span>{feature}</span>
                      </motion.li>
                    ))}
                  </ul>
                  ...
                  <Link to={service.link} className="inline-block">
                    <button className="text-white font-semibold flex items-center gap-1 sm:gap-2 group-hover:gap-3 transition-all text-sm sm:text-base hover:text-blue-300">
                      Learn More 
                      <ChevronRightIcon className="w-4 h-4 sm:w-5 sm:h-5 transform group-hover:translate-x-2 transition-transform duration-300" />
                    </button>
                  </Link>
                </div>
                
                {/* Hover Border Effect */}
                <div className="absolute inset-0 border-2 border-transparent group-hover:border-white/30 rounded-xl sm:rounded-2xl transition-all duration-500 pointer-events-none"></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 md:gap-8">
            {filteredProjects.map((project, index) => (
              <motion.div
                ...
                className="group bg-white rounded-xl sm:rounded-2xl overflow-hidden shadow-lg hover:shadow-xl md:hover:shadow-2xl transition-all duration-300 cursor-pointer"
              >
                <div className="relative h-48 sm:h-56 md:h-64 overflow-hidden">
                  ...
                  <div className="absolute top-2 sm:top-3 md:top-4 right-2 sm:right-3 md:right-4 bg-gradient-to-r from-blue-600 to-cyan-500 text-white px-2 py-1 sm:px-3 sm:py-1.5 md:px-4 md:py-2 rounded-full text-xs sm:text-sm font-semibold transform group-hover:scale-110 transition-transform duration-300">
                    {project.category}
                  </div>
                  <div className="absolute bottom-2 sm:bottom-3 md:bottom-4 left-2 sm:left-3 md:left-4">
                    <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-white group-hover:text-blue-200 transition-colors duration-300">
                      {project.title}
                    </h3>
                ...
                <div className="p-4 sm:p-5 md:p-6">
                  <div className="flex flex-wrap gap-1 sm:gap-2 mb-3 sm:mb-4">
                    {project.technologies.map((tech, idx) => (
                      <span 
                        key={idx} 
                        className="px-2 py-0.5 sm:px-3 sm:py-1 bg-gradient-to-r from-blue-50 to-cyan-50 text-blue-600 rounded-full text-xs sm:text-sm font-medium group-hover:from-blue-100 group-hover:to-cyan-100 transition-all duration-300"
                      >
                        {tech}
                      </span>
                    ))}
                  ...
                  <div className="flex justify-between items-center">
                    <div>
                      <div className="text-gray-600 font-medium text-xs sm:text-sm md:text-base group-hover:text-gray-800 transition-colors duration-300">
                        {project.category}
                      </div>
                    </div>
                    <button className="text-blue-600 hover:text-blue-700 flex items-center gap-1 sm:gap-2 font-semibold group-hover:gap-3 transition-all text-xs sm:text-sm md:text-base">
                      View Details 
                      <ExternalLink className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 transform group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform duration-300" />
                    </button>
                  </div>
                </div>
                
                {/* Hover Overlay */}
                <div className="absolute inset-0 bg-gradient-to-r from-blue-600/10 to-cyan-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"></div>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-10 sm:py-14 md:py-20 bg-gradient-to-b from-gray-50 to-white">
        <div className="container mx-auto px-4 sm:px-6">
          ...
          {/* Distributed Logo Grid with Hover Effects */}
          <div className="relative">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4 md:gap-6 mb-6 sm:mb-8">
              {clientLogos.slice(0, 6).map((client, index) => (
                ...
                >
                  <div className="bg-white p-4 sm:p-5 md:p-6 rounded-xl sm:rounded-2xl shadow-md hover:shadow-lg md:hover:shadow-xl transition-all duration-300 h-20 sm:h-24 md:h-32 flex flex-col items-center justify-center border border-gray-200 hover:border-blue-300">
                    <div className="relative w-full h-6 sm:h-8 md:h-10 flex items-center justify-center mb-1 sm:mb-2 md:mb-3">
                      <img
                        src={client.logo}
                        alt={client.name}
                        className="max-w-full max-h-6 sm:max-h-8 md:max-h-10 object-contain grayscale group-hover:grayscale-0 transition-all duration-300 group-hover:scale-110"
                      />
                    </div>
                    <div className="text-center">
                      <div className="text-xs sm:text-sm font-medium text-gray-700 group-hover:text-blue-600 transition-colors duration-300">
                        {client.name}
                      </div>
                    </div>
                  </div>
            ...
            {/* Center Stats */}
            <div className="mt-10 sm:mt-12 md:mt-16 bg-gradient-to-r from-blue-600 to-cyan-600 rounded-xl sm:rounded-2xl md:rounded-3xl p-6 sm:p-8 md:p-12 shadow-xl md:shadow-2xl hover:shadow-2xl md:hover:shadow-3xl transition-all duration-300 cursor-pointer hover:-translate-y-1">
      ...
      <section className="py-10 sm:py-14 md:py-20 bg-gradient-to-br from-blue-50 via-white to-cyan-50">
        <div className="container mx-auto px-4 sm:px-6">
          ...
          {/* Single Review Carousel with Hover Effects */}
          <div className="relative max-w-6xl mx-auto">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="bg-white rounded-xl sm:rounded-2xl md:rounded-3xl shadow-lg md:shadow-2xl overflow-hidden border border-gray-200 hover:shadow-xl md:hover:shadow-3xl transition-all duration-300 cursor-pointer"
              >
                <div className="grid grid-cols-1 lg:grid-cols-3">
                  {/* Left Side - Client Info */}
                  <div className="bg-gradient-to-br from-blue-600 to-cyan-500 p-6 sm:p-8 md:p-12 text-white hover:from-blue-700 hover:to-cyan-600 transition-all duration-500">
                    <div className="flex flex-col items-center text-center h-full justify-center">
                      <div className="relative mb-4 sm:mb-6 group">
                        <img
                          src={testimonials[testimonialIndex].image}
                          alt={testimonials[testimonialIndex].name}
                          className="w-20 h-20 sm:w-24 sm:h-24 md:w-32 md:h-32 rounded-full border-2 sm:border-3 md:border-4 border-white/30 shadow-lg md:shadow-2xl group-hover:border-white/50 transition-all duration-300"
                        />
                        <div className="absolute -bottom-1 sm:-bottom-2 -right-1 sm:-right-2 bg-gradient-to-r from-yellow-400 to-amber-500 text-white p-1 sm:p-2 rounded-full group-hover:scale-110 transition-transform duration-300">
                          <Star className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 fill-current" />
                        </div>
                      </div>
                      
                      <h3 className="text-lg sm:text-xl md:text-2xl font-bold mb-1 sm:mb-2 group-hover:text-blue-100 transition-colors duration-300">
                        {testimonials[testimonialIndex].name}
                      </h3>
                      <p className="text-blue-100 font-medium text-xs sm:text-sm md:text-base mb-1 group-hover:text-white transition-colors duration-300">
                        {testimonials[testimonialIndex].role}
                      </p>
                      <div className="flex items-center gap-1 sm:gap-2 text-blue-100 mb-3 sm:mb-4 group-hover:text-white transition-colors duration-300">
                      ...
                      <div className="flex mb-3 sm:mb-4">
                        {[...Array(testimonials[testimonialIndex].rating)].map((_, i) => (
                          <Star 
                            key={i} 
                            className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-yellow-300 fill-current hover:scale-125 transition-transform duration-300" 
                          />
                        ))}
                      </div>
                      
                      <div className="space-y-2 sm:space-y-3 mt-4 sm:mt-6">
                        <div className="flex items-center gap-1 sm:gap-2 bg-white/10 px-3 py-1 sm:px-4 sm:py-2 rounded-full hover:bg-white/20 transition-all duration-300">
                  ...
                  <div className="lg:col-span-2 p-6 sm:p-8 md:p-12 hover:bg-gray-50/50 transition-all duration-300">
                    <div className="h-full flex flex-col justify-center">
                      <div className="relative mb-6 sm:mb-8">
                        <div className="absolute -top-4 sm:-top-6 -left-4 sm:-left-6 md:-top-8 md:-left-8 text-blue-100 group-hover:text-blue-200 transition-colors duration-300">
                          <Quote className="w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16" />
                        </div>
                        <p className="text-base sm:text-lg md:text-xl lg:text-2xl md:text-3xl font-light text-gray-600 leading-relaxed italic pl-6 sm:pl-8 group-hover:text-gray-700 transition-colors duration-300">
                          "{testimonials[testimonialIndex].content}"
                        </p>
                      </div>
                      
                      <div className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-gray-200 group-hover:border-gray-300 transition-colors duration-300">
                        <div className="flex flex-wrap gap-2 sm:gap-3 md:gap-4">
                          <div className="flex items-center gap-1 sm:gap-2 px-3 py-1.5 sm:px-4 sm:py-2 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg hover:from-green-100 hover:to-emerald-100 transition-all duration-300">
                            <CheckCircle className="w-4 h-4 sm:w-5 sm:h-5 text-green-500" />
                            <span className="font-semibold text-green-700 text-xs sm:text-sm md:text-base">Project Success</span>
                          </div>
                          <div className="flex items-center gap-1 sm:gap-2 px-3 py-1.5 sm:px-4 sm:py-2 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-lg hover:from-blue-100 hover:to-cyan-100 transition-all duration-300">
                            <TrendingUp className="w-4 h-4 sm:w-5 sm:h-5 text-blue-500" />
                            <span className="font-semibold text-blue-700 text-xs sm:text-sm md:text-base">Positive ROI</span>
                          </div>
                          <div className="flex items-center gap-1 sm:gap-2 px-3 py-1.5 sm:px-4 sm:py-2 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg hover:from-purple-100 hover:to-pink-100 transition-all duration-300">
            ...
            <div className="flex items-center justify-center mt-8 sm:mt-10 md:mt-12 space-x-4 sm:space-x-6 md:space-x-8">
              <button
                ...
                className="flex items-center gap-2 sm:gap-3 px-4 py-2 sm:px-5 sm:py-2.5 md:px-6 md:py-3 bg-gradient-to-r from-blue-600 to-cyan-500 text-white rounded-full font-semibold hover:shadow-lg transition-all duration-300 hover:scale-105 text-xs sm:text-sm md:text-base hover:from-blue-700 hover:to-cyan-600"
              >
            ...
            {/* Review Stats with Hover Effects */}
            <div className="mt-10 sm:mt-12 md:mt-16 grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
              <motion.div 
                className="text-center p-4 sm:p-5 md:p-6 bg-white rounded-lg sm:rounded-xl md:rounded-2xl shadow-md sm:shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 cursor-pointer"
                whileHover={{ scale: 1.05 }}
              >
      ...
      {/* Final CTA Section with Hover Effects */}
        ...
        <div className="container mx-auto px-4 sm:px-6">
          <div className="max-w-4xl mx-auto text-center">
            <motion.div
              ...
              viewport={{ once: true }}
              whileHover={{ scale: 1.02 }}
              className="bg-white rounded-xl sm:rounded-2xl md:rounded-3xl p-6 sm:p-8 md:p-12 border border-gray-300 shadow-xl hover:shadow-2xl transition-all duration-300"
            >
              <h3 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-gray-800 mb-4 sm:mb-6">
                Ready to Transform Your Business?
              </h3>
              <p className="text-gray-600 mb-6 sm:mb-8 text-sm sm:text-base md:text-lg max-w-2xl mx-auto">
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
                <Link to="/contact">
                  <motion.button 
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    className="bg-gradient-to-r from-blue-600 to-cyan-500 text-white px-6 py-3 sm:px-8 sm:py-3.5 md:px-10 md:py-4 rounded-lg sm:rounded-xl font-bold hover:shadow-xl md:hover:shadow-2xl transition-all duration-300 transform flex items-center gap-2 text-sm sm:text-base md:text-lg hover:from-blue-700 hover:to-cyan-600"
                  >
