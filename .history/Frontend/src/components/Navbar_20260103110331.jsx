...
const Navbar = () => {
  ...
  const navigate = useNavigate();  // useNavigate hook लिया
  ...
  // Mobile service click handler
  const handleMobileServiceClick = (path) => {
    setIsOpen(false);
    setServiceOpen(false);
    navigate(path);  // useNavigate का उपयोग किया
  };
                ...
                {serviceOpen && (
                  <div className="ml-4 mt-3 space-y-2 max-h-[60vh] overflow-y-auto pr-2 animate-fade-in">
                    {services.map((service) => (
                      <div
                        key={service.name}
                        onClick={() => handleMobileServiceClick(service.path)}  // यहाँ function call किया
